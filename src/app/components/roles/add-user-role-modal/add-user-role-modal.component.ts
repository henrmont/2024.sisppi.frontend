...
@Component({
  ...
  styleUrl: './add-user-role-modal.component.scss'
})
export class AddUserRoleModalComponent {
  ...
  formulario: FormGroup = this.formBuilder.group({
    role_id: [this.data.info.id, Validators.required],
    user_id: [null, Validators.required],
  });

  ngOnInit(): void {
    this.userService.getUsersWithoutRole(this.data.info.id).subscribe({
      next: (response: any) => {
  ...
  onSubmit() {
    this.roleService.addUserRole(this.formulario.value).subscribe({
      next: (response: any) => {
